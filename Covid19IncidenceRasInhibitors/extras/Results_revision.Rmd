...
\centerline{{\Huge Tables and Figures}}
...
\begin{table}[H]
 \caption{
Populations and COVID-19 diagnoses for angiotensin converting enzyme inhibitor (ACEI), angiotensin receptor blocker (ARB), calcium channel block (CCB) and thiazide or thiazide-like diuretics (THZ) monotherapy and in-combination user cohorts. For each target (T) and comparator (C) cohort, we report population size, total exposure time, outcome events and minimally detectable risk ratio (MDRR).%
}
...
print(xtable(table, format = "latex"),
      ...
      add.to.row = list(
        ...
        command = c(
          "                       \\multicolumn{9}{l}{ACEI/ARB vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{9}{l}{ACEI vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{9}{l}{ARB vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{9}{l}{ACEI vs ARB} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\"          
          )
      ),
...
cuimcTc <- tableCuimc %>% head(1) %>% select(targetSubjects, comparatorSubjects)
  ...
  scale <- 0.8
  header <- readr::read_file("Table1TopTriple_revision.tex")
  ...
  title <- gsub("% Another row",
                paste0("& \\\\multicolumn{6}{c}{$N$ = ", sidiapTc$targetSubjects, " (T) and ", sidiapTc$comparatorSubjects, " (C) }",
                      "& \\\\multicolumn{6}{c}{$N$ = ", vaTc$targetSubjects, " (T) and ", vaTc$comparatorSubjects, " (C) }",
                      "& \\\\multicolumn{6}{c}{$N$ = ", cuimcTc$targetSubjects, " (T) and ", cuimcTc$comparatorSubjects, " (C) }",
                      " \\\\\\\\"),
               x = title)
...
\begin{table}
 \caption{Relative risk of COVID-19 diagnosis for ACEI, ARB, CCB and THZ prevalent-use. We report calibrated hazard ratios (HRs) and their 95\% confidence intervals (CIs) and calibrated p-value (P) for both monotherapy and in-combination cohort definitions, with propensity score (PS) stratification or matching and across data sources. Grayed out data source entries do not pass study diagnostics and grayed out meta-analysis entries 
 return heterogeneity measure $i^2$ >0.4.}
  \rowcolors{2}{gray!6}{white}
  \centering{
...
print(xtable(annotatedWideTable %>% filter(outcomeId == covidNarrowId) %>% 
 ...
 add.to.row = list(
        pos = as.list(c(0:7) * numDb),
        command = c(
          "                       \\multicolumn{10}{l}{ACEI/ARB vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{10}{l}{ACEI vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{10}{l}{ARB vs CCB/THZ} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\",
          "\\rowcolor{white} \\\\ \\multicolumn{10}{l}{ACEI vs ARB} \\\\ & \\multicolumn{9}{l}{Monotherapy}  \\\\",
          "                        & \\multicolumn{9}{l}{+ Combination} \\\\"          
          )
      ),      
      sanitize.text.function = identity)
...
cohortNames <- c("ACEI/ARB vs CCB/THZ", "ACEI vs CCB/THZ", "ARB vs CCB/THZ", "ACEI vs ARB")
...
\begin{figure}[H]
  \caption{
International Covid-ACE Receptor Inhibition Utilization and Safety (ICARIUS) susceptibility study design schematic. We highlight eligibility criteria, exposure definitions, adjustment strategies, index date specification (horizontal black arrow) and outcome definitions and time-at-risk. Exposure involves prescriptions to drugs with RxNorm ingredients that map to the first-line antihypertensive angiotensin converting enzyme inhibitor (ACEI), angiotensin receptor blocker (ARB), dihydropyridine calcium channel blockers (CCB) and thiazide or thiazide-like diuretic (THZ) classes.  
  }
  \centerline{
...
\begin{figure}[H]
  \caption{
Relative risk of COVID-19-related outcomes for ACEI, ARB, CCB and THZ prevalent-use across data sources. Outcomes are COVID-19 diagnosis, COVID-19 hospitalization (+Hospital), patients hospitalized with pneumonia (PNA) and patients hospitalized with pneumonia, acute respiratory distress syndrome, acute kidney injury or sepsis (PAAS). We plot calibrated hazard ratios (circles) and their 95\% confidence intervals labeled by propensity score (PS) adjustment method (black/white). 
Grayed out data source entries do not pass study diagnostics and grayed out meta-analysis entries return heterogeneity measure $i^2$ >0.4.
}
    \vspace*{-0.5in}
  \centerline{
