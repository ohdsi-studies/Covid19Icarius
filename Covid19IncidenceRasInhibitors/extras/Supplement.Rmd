...
makeSideBySideBalancePlots <- function(databaseId, covariate,
...
saveSideBySidePlots <- function(plot) {
  ...
  invisible(save_plot(file,
            plot_grid(plot$plotStrat, plot$plotMatch, plot$psStrat),
            ncol = 2, base_asp = 0.5, base_height = 8))
...
devNull <- lapply(tcs, function(tc) {
  invisible(info <- saveSideBySidePlots(
    makeSideBySideBalancePlots("SIDIAP", sidiapCovariate, tc$targetId, tc$comparatorId, covidNarrowId,
                               studyFolder)
  ))
...
devNull <- lapply(tcs, function(tc) {
  invisible(info <- saveSideBySidePlots(
    makeSideBySideBalancePlots("CUIMC", cuimcCovariate, tc$targetId, tc$comparatorId, covidNarrowId,
                               studyFolder)
  ))
