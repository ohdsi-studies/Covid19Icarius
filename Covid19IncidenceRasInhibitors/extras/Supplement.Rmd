...
\rowcolors{2}{gray!6}{white}
\begin{longtable}{p{-2em}p{-2em}rrrrrrrr}
 \caption{Population size, total exposure time and intensive services (events) for ACEI, ARB and CCB/THZ monotherapy and in-combination user target (T) and comparator (C) cohorts.}
  \\
    \hiderowcolors
    \toprule
    &  &  & \multicolumn{2}{c}{Patients} & \multicolumn{2}{c}{Time (years)} & \multicolumn{2}{c}{Events} \\
...
# Baseline patient characteristics and balance
...
saveSideBySidePlots <- function(plot) {
  ...
  invisible(save_plot(file,
            plot_grid(
              plot$psStrat, plot$plotStrat, plot$plotMatch, plot$errorStrat, plot$errorMatch),
            ncol = 1, base_asp = 0.5, base_height = 8))
...
devNull <- lapply(tcs, function(tc) {
  invisible(info <- saveSideBySidePlots(
    makeSideBySideBalancePlots("SIDIAP", sidiapCovariate, sidiapResults,
