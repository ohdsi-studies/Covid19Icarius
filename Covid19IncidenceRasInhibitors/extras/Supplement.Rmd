...
colnames(cmResults) <- SqlRender::snakeCaseToCamelCase(colnames(cmResults))
cmResults <- cmResults %>% filter(outcomeId %in% outcomesOfInterest, analysisId == primaryAnalysisId)
...
table <- do.call(
  ...
  lapply(tcs, function(tc) {
    cmResults %>% filter(targetId == tc$targetId, comparatorId == tc$comparatorId) %>%
      ...
      select(targetName, comparatorName, outcomeId, 
             rr, ci, p, 
...
\begin{table}[H]
 \caption{Need a title for COVID narrow effects in SIDIAP. (+) indicates in-combination.}
  \rowcolors{2}{gray!6}{white}
  \centering{
  \begin{tabular}{llrrrrrr}
    ...
    \midrule \showrowcolors
```{r, echo=FALSE, results="asis"}
print(xtable(table %>% filter(outcomeId == covidNarrowId) %>% select(-outcomeId), 
             format = "latex"),
      include.rownames = FALSE,
      include.colnames = FALSE,
      hline.after = NULL,
